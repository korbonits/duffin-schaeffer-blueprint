...
\begin{proposition}[Divergence Borel--Cantelli, quasi-independent form]
  \label{prop:quasi-bc}
  \notready
  \uses{prop:quasi-bc-escaping}
  As above, but assuming only that the first moments $\sum_{i \in S_n} \mu(A_i)$ tend to
  infinity, with no escape hypothesis.
\end{proposition}

\begin{proof}
  The reduction to Proposition \ref{prop:quasi-bc-escaping} is worth writing down,
  because the escape hypothesis is not obviously removable.

  Fix a finite $t$ and put $S_n' = S_n \setminus t$. The overlap bound for $S_n'$ is
  inherited (the double sum only shrinks), and with $x = \sum_{i \in S_n'} \mu(A_i)$ and
  $k = \#t$ we have $\sum_{i \in S_n} \mu(A_i) \leq x + k$, so Chung--Erd\H{o}s on
  $S_n'$ gives
  \[ x^2 \leq M \cdot C \cdot (x+k)^2, \qquad M = \mu\Big(\bigcup_{i \notin t} A_i\Big). \]
  Since $x \to \infty$ and $k$ is fixed, $(x/(x+k))^2 \to 1$, so $MC \geq 1$, i.e.
  $C^{-1} \leq M$. Then intersect over $t$ as before.

  The awkwardness is entirely $\mathbb{R}_{\geq 0}^{\infty}$: the ratio manipulation
  wants division and a limit, and every quantity here is finite, so the sensible move is
  to pass to $\mathbb{R}_{\geq 0}$ first.

  Alternatively, if Theorem \ref{thm:overlap-core} can be strengthened to produce
  escaping $S_n$ --- Koukoulopoulos and Maynard select denominators from ranges tending
  to infinity, so it should --- this node is not needed at all. That has not been checked
  against the paper.
\end{proof}
