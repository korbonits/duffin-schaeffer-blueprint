...
\begin{definition}[The majorant $\pstar$]
  \label{def:psistar}
  \lean{DuffinSchaeffer.psiStar}
  \leanok
  $\pstar(q) = \varphi(q) \sup \{ \ps(n)/n : q \mid n \}$.
\end{definition}
...
\begin{lemma}[Measure of a single $\Aq$]
  ...
  \notready
  \uses{def:setaq}
  $\Leb(\Aq) \leq 2\ps(q)\varphi(q)/q$, with equality when $\ps(q) \leq 1/2$.
\end{lemma}

\begin{proof}
  Each of the $\varphi(q)$ admissible numerators contributes an interval of length
  $2\ps(q)/q$. For $\ps(q) \leq 1/2$ these intervals are pairwise disjoint, since
  consecutive reduced fractions with denominator $q$ are at distance at least $1/q$.
  Lean: \texttt{volume\_setAq\_le}, \texttt{volume\_setAq}.
\end{proof}
...
\begin{lemma}[Divergence transfers to the measures]
  ...
  $\sum_q \Leb(\Aq) = \infty$. Lean: \texttt{not\_summable\_volume\_setAq}.
\end{lemma}
...
\begin{lemma}[Average order of $\varphi$]
  ...
  \notready
  $\sum_{q \leq n} \varphi(q)/q \asymp n$. Lean: \texttt{sum\_totient\_div\_self}.
\end{lemma}
...
\begin{theorem}[Theorem 2(a), convergence]
  ...
  If $\sum_q \pstar(q) < \infty$ then $\Leb(\K(\ps)) = 0$.
\end{theorem}

\begin{proof}
  The first Borel--Cantelli lemma, which Mathlib has as
  \texttt{MeasureTheory.measure\_limsup\_cofinite\_eq\_zero}, applied to the cover of
  $\K(\ps)$ by the sets attached to $\pstar$. This node needs nothing from Chapters
  \ref{chap:overlap} or \ref{chap:gcd} and is the recommended first real proof in the
  project. Lean: \texttt{theorem\_2\_a}.
\end{proof}
