...
\begin{lemma}[Reduction to $\ps \leq 1/2$, convergent case]
  \label{lem:truncation-gap}
  \notready
  \uses{lem:seta-mono}
  \textbf{TRANSCRIBE} --- paper \S1.

  If $\sum \ps(q)\varphi(q)/q$ diverges while $\sum \min(\ps,1/2)(q)\varphi(q)/q$
  converges, then $\Leb(\A(\ps)) = 1$.
\end{lemma}

\begin{proof}
  This gap sat inside the prose of Theorem \ref{thm:one} until the surrounding nodes were
  closed, so it is recorded as a node rather than left as a sentence.

  Write $S = \{q : \ps(q) > 1/2\}$. On $S$ the truncation equals $1/2$, so convergence of
  the truncated series forces $\sum_{q \in S} \varphi(q)/q < \infty$; off $S$ the two
  functions agree, so the divergence comes entirely from $S$:
  \[ \sum_{q \in S} \frac{\ps(q)\varphi(q)}{q} = \infty, \qquad
     \sum_{q \in S} \frac{\varphi(q)}{q} < \infty . \]
  So $\ps$ is unbounded along $S$ in a weighted sense and the truncated series carries
  none of the divergence.

  Nothing in this development settles whether $\A(\ps)$ is full in that regime.
  $\ps(q) \geq q$ infinitely often would give it at once, since then $\Aq = [0,1]$; but
  $\ps(q)$ can be large without $\ps(q)/q$ being large, and the count $\varphi(q)$ of
  admissible numerators does not force a cover --- at $q = 6$ the balls around $1/6$ and
  $5/6$ cover $[0,1]$ only once $\ps(6) \geq 2$. Lean:
  \texttt{volume\_setA\_eq\_one\_of\_untruncated}.
\end{proof}
