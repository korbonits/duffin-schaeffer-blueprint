...
\begin{proof}
  The null set is no longer the open question --- Lemma \ref{lem:strict} disposes of it.
  What remains is genuinely fiddly rather than deep.
  \texttt{UnitAddCircle.mem\_addWellApproximable\_iff} expresses membership as
  $\exists m < n,\ \gcd(m,n) = 1,\ \|x - m/n\| < \delta_n$, so one has to match $m < n$
  against coprime $a \leq q$ (they agree: $a = q$ forces $q = 1$, and $1/1 \equiv 0/1$ on
  the circle), and match the quotient norm against $|\cdot|$, which for
  $\alpha \in [0,1]$ and radius at most $1/(2q)$ means checking that only the
  representatives $a/q$ and $a/q \pm 1$ can be nearest.
  Lean: \texttt{setA\_eq\_preimage\_addWellApproximable}.
\end{proof}
